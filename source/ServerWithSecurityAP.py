                ...
                while True:
                    match convert_bytes_to_int(read_bytes(client_socket, 8)):
                        ...
                        case 3:
                            ...
                            print("Authenticating certificate")

                            #Read M1 from socket
                            M1 = client_socket.recv(8)
                            #Convert M1 to int to set how many bytes to read M2
                            M2_size = convert_bytes_to_int(M1)
                            print("M2 size: " + str(M2_size))
                            #Read M2
                            M2 = client_socket.recv(M2_size)

                            print("first msg read")
                            
                            # Get server private key Ks- from server_private_key.pem
                            try:
                                with open(file="auth/server_private_key.pem", mode="r", encoding="utf8") as key_file:
                            ...
                            # Encrypt M using server private key Ks-
                            ...
                            # Send signed_message size and signed_message to client
                            client_socket.sendall(convert_int_to_bytes(len(signed_message)))
                            client_socket.sendall(signed_message)

                            # Send server_signed.crt size and server_signed.crt to client
                            server_signed_file = "source\auth\server_signed.crt"
                            file_size = sys.getsizeof(server_signed_file)
                            client_socket.sendall(convert_int_to_bytes(file_size))
                            client_socket.sendfile(server_signed_file)
