...
def main(args):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            sess_keys_bytes = Fernet.generate_key() 
            sess_keys = Fernet(sess_keys_bytes)
            #sharing of session key protocol
            s.sendall(convert_int_to_bytes(4)) #MODE:4
            s.sendall(convert_int_to_bytes(len(sess_keys_bytes)))
            s.sendall(sess_keys_bytes)
            print("Session key is sent.")
            ...
            with open(filename, mode="rb") as fp:
                long_message = fp.read()
                encrpt_long_message = sess_keys.encrypt(long_message)
                s.sendall(convert_int_to_bytes(len(encrpt_long_message)))
                s.sendall(encrpt_long_message)
            #administrative work
            with open(f"send_files_enc/enc_{filen}",mode = "wb") as f:
                f.write(encrpt_long_message)
